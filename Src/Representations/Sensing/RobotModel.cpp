...
RobotModel::RobotModel(const JointAngles &jointAngles,
...
void RobotModel::setJointData(const JointAngles &jointAngles,
                              const RobotDimensions &robotDimensions,
                              const MassCalibration &massCalibration) {
  ForwardKinematic::calculateHeadChain(jointAngles, robotDimensions, limbs);
  ForwardKinematic::calculateArmChain(Arms::left, jointAngles, robotDimensions,
                                      limbs);
  ForwardKinematic::calculateArmChain(Arms::right, jointAngles, robotDimensions,
                                      limbs);
  ForwardKinematic::calculateLegChain(Legs::left, jointAngles, robotDimensions,
                                      limbs);
  ForwardKinematic::calculateLegChain(Legs::right, jointAngles, robotDimensions,
                                      limbs);

  soleLeft =
      limbs[Limbs::footLeft] + Vector3f(0.f, 0.f, -robotDimensions.footHeight);
  soleRight =
      limbs[Limbs::footRight] + Vector3f(0.f, 0.f, -robotDimensions.footHeight);
