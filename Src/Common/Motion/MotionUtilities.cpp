...
void MotionUtilities::copy(const JointRequest &source, JointRequest &target,
                           ...
                           const Joints::Joint startJoint,
                           const Joints::Joint endJoint) {
  for (int i = startJoint; i <= endJoint; ++i) {
    if (source.angles[i] != JointAngles::ignore)
      target.angles[i] = source.angles[i];
    target.stiffnessData.stiffnesses[i] =
        target.angles[i] != JointAngles::off
            ? source.stiffnessData.stiffnesses[i]
            : 0;
...
void MotionUtilities::interpolate(const JointRequest &from,
                                  const JointRequest &to, float fromRatio,
                                  JointRequest &target,
                                  bool interpolateStiffness,
                                  const StiffnessSettings &theStiffnessSettings,
                                  const JointAngles &lastJointAngles,
                                  const Joints::Joint startJoint,
                                  const Joints::Joint endJoint) {
  for (int i = startJoint; i <= endJoint; ++i) {
    ...
    if (t == JointAngles::ignore && f == JointAngles::ignore) continue;

    if (t == JointAngles::ignore) t = target.angles[i];
    if (f == JointAngles::ignore) f = target.angles[i];

    int fStiffness =
        f != JointAngles::off ? from.stiffnessData.stiffnesses[i] : 0;
    int tStiffness =
        t != JointAngles::off ? to.stiffnessData.stiffnesses[i] : 0;
    ...
    if (target.angles[i] == JointAngles::off ||
        target.angles[i] == JointAngles::ignore)
      target.angles[i] = lastJointAngles.angles[i];

    assert(target.angles[i] != JointAngles::off &&
           target.angles[i] != JointAngles::ignore);
    ...
    if (interpolateStiffness)
      target.stiffnessData.stiffnesses[i] +=
          int(-fromRatio * float(tStiffness) + fromRatio * float(fStiffness));
    else
      target.stiffnessData.stiffnesses[i] = tStiffness;
  }
}

bool MotionUtilities::interpolate(JointRequest &joints, const float alpha,
  ...
  JointAngles lastAngles =
      theJointRequest.isValid() ? theJointRequest : theJointAngles;
  float diff = 0;

  for (int j = 0; j < Joints::numOfJoints; j++) {
    if (lastAngles.angles[j] == JointAngles::off ||
        lastAngles.angles[j] == JointAngles::ignore)
      lastAngles.angles[j] = theJointAngles.angles[j];

    if (theStandLegRequest.angles[j] == JointAngles::off ||
        theStandLegRequest.angles[j] == JointAngles::ignore)
      joints.angles[j] = lastAngles.angles[j];
    else {
      joints.angles[j] = lastAngles.angles[j] * (1.f - alpha) +
                         theStandLegRequest.angles[j] * alpha;
...
void MotionUtilities::interpolate(const JointAngles &from,
                                  const JointRequest &to, float &ratio,
                                  JointRequest &target,
                                  const JointAngles &theJointAngles) {
